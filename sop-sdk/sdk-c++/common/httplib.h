...
public:
  ...
  std::shared_ptr<Response> Get(const char *path, std::map<std::string, std::string> params, const Headers &headers);
...
inline bool Client::write_request(Stream &strm, const Request &req,
  ...
  // Request line
    std::string path = detail::encode_url(req.path);
  if (!req.query.empty()) {
    path = path + "?" + req.query;
  }
...
inline
std::shared_ptr<Response>
Client::Get(const char *path, std::map<std::string, std::string> params, const Headers &headers) {
    Request req;
    req.method = "GET";
    req.path = path;
    req.headers = headers;
    ...
    for(iter = params.begin(); iter != params.end(); iter++) {
        query.append("&").append(iter->first + "=" + detail::encode_url(iter->second));
    }
    req.query = query;
